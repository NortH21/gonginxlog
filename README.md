...
По умолчанию gonginxlog печатает агрегированный отчёт (в терминале —
статус-коды подсвечены цветом: 2xx зелёный, 3xx голубой, 4xx жёлтый,
5xx красный; `NO_COLOR=1` отключает):

- распределение по кодам статуса
- топ клиентских IP, стран, запрошенных путей (`--top N` строк для
...
- таблица "Upstreams" — куда шёл трафик, средний request_time и доля
  upstream-статусов 5xx на каждый бэкенд; появляется автоматически,
  только если в log_format есть `$upstream_addr` (запросы вообще без
  апстрима — кэш/статика — считаются в строке `-`). При ретраях
  (`$upstream_addr`/`$upstream_status`/`$upstream_response_time`
  через запятую) в статистику идёт **последнее** значение — тот
  бэкенд, что реально ответил клиенту, а не все подряд и не первый
...
Пути/UA/referer'ы и сырые строки (`--lines`, `-f`, `--ui`) — это данные
из настоящих HTTP-запросов, т.е. их полностью контролирует клиент.
Перед печатью в терминал/TUI они всегда очищаются от управляющих
байтов (в т.ч. ANSI escape-последовательностей) и от tview-разметки —
это не отключается и не настраивается флагами, так безопаснее.
...
есть geoip) · `4` paths · `5` agents (только с `--show-agents`) ·
`6` referers (только с `--show-referers`) · `7` timeline · `8` routes
(только с `--routes-file`) · `l` raw (живые совпавшие строки) ·
`a` alerts · `/` фильтр · `x` сброс фильтра · `p` пауза/продолжить
обновление (только live) · `Enter` drill-down по выбранной строке ·
`Esc` назад/закрыть · `q` выход · `j`/`k` работают как стрелки
вниз/вверх. Футер всегда показывает только реально доступные в данный
момент хоткеи.
...
- Таблица "Upstreams" пока только в batch-отчёте — в `--ui` (вкладка
  или drill-down-измерение) сознательно не добавлена, чтобы не грузить
  живой дашборд лишним, пока нет конкретного запроса.
