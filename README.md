...
батчем (чтобы дашборд не был пустым), затем переходит в живой тайлинг. В
хедере всегда показывается версия сборки (`gonginxlog vX.Y.Z`) — удобно
...
фильтра · `p` пауза/продолжить обновление · `Enter` drill-down по
выбранной строке · `Esc` назад/закрыть · `q` выход · `j`/`k` работают
как стрелки вниз/вверх.

**Пауза (`p`)** останавливает обновление экрана (в хедере `● LIVE`
сменяется на `⏸ PAUSED`) — удобно, чтобы спокойно посмотреть текущие
цифры или скопировать строки, не борясь с ежесекундным перерисовыванием.
Данные при этом **не теряются**: тайлинг и агрегация продолжаются в
фоне, при повторном `p` экран сразу подхватывает всё, что накопилось за
время паузы.
...
**Alerts (`a`)** показывает аномалии от трёх детекторов: один IP
генерирует большую долю трафика за последние 60с (флуд), один IP
касается многих разных путей за последние 10с (скан), один путь
получает запросы от многих разных IP за последние 10с (распределённый
перебор). В хедере показывается бейдж `⚠ N alert(s)`, когда есть
активные алерты.

Пороги настраиваются флагами — дефолты не универсальны: у одного сайта
30% трафика с одного IP за минуту — это флуд, у другого — это
легитимный health-check или один активный клиент игрового бэкенда:

```sh
gonginxlog --ui --anomaly-ip-share 0.6 --anomaly-ip-min 50 \
  --anomaly-scan-paths 40 --anomaly-hammer-ips 30 access.log
```

| Флаг | Что значит | По умолчанию |
|---|---|---|
| `--anomaly-ip-share` | доля трафика (0–1) с одного IP за 60с для флуда | `0.3` |
| `--anomaly-ip-min` | минимум запросов в этом окне, прежде чем долю вообще проверяют | `20` |
